...
object LBFGS extends Logging {
  ...
  def runLBFGS(
    ...
    val states =
      lbfgs.iterations(new CachedDiffFunction(costFun), SparkUtils.toBreeze(initialWeights).toDenseVector)
    ...
    lossHistory += state.value
    val weights = SparkUtils.fromBreeze(state.x)
  ...
  private class CostFun(
    ...
    override def calculate(weights: BDV[Double]): (Double, BDV[Double]) = {
      ...
      val w = SparkUtils.fromBreeze(weights)
      val n = w.size
      ...
      (loss, SparkUtils.toBreeze(gradientTotal).asInstanceOf[BDV[Double]])
    }
  }
}
