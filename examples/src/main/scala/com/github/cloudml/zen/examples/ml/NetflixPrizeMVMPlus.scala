...
object NetflixPrizeMVMPlus extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("MVMPlus") {
      head("NetflixPrizeMVMPlus: an example app for MVMPlus.")
      ...
      note(
        """
          ...
          |
          | bin/spark-submit --class com.github.cloudml.zen.examples.ml.NetflixPrizeMVMPlus \
  ...
  def run(params: Params): Unit = {
    ...
    SparkHacker.gcCleaner(60 * 10, 60 * 10, "NetflixPrizeMVMPlus")
    val (trainSet, testSet, views) = NetflixPrizePartialUtils.genSamplesWithTime(sc, input, numPartitions)
    val fm = new MVMPlusRegression(trainSet, stepSize, views, regular, 0.0, rank,
