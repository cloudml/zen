...
object MovieLensMVMPlus extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("MVMPlus") {
      head("MovieLensMVMPlus: an example app for MVMPlus.")
      ...
      note(
        """
          ...
          |
          | bin/spark-submit --class com.github.cloudml.zen.examples.ml.MovieLensMVMPlus \
  ...
  def run(params: Params): Unit = {
    ...
    val conf = new SparkConf().setAppName(s"MVMPlus with $params")
    if (kryo) {
    ...
    SparkHacker.gcCleaner(60 * 10, 60 * 10, "MovieLensMVMPlus")
    ...
    val lfm = new MVMPlusRegression(trainSet, stepSize, views, regular, 0.0, rank,
      useAdaGrad, useWeightedLambda, 1, storageLevel)
    var iter = 0
    var model: MVMPlusModel = null
