 ...
 */
class GraphXPrimitiveVector[@specialized(Long, Int, Double) V: ClassTag](initialSize: Int = 64) {
  ...
  def trim(): GraphXPrimitiveVector[V] = resize(size)

  /** Resizes the array, dropping elements if the total length decreases. */
  def resize(newLength: Int): GraphXPrimitiveVector[V] = {
